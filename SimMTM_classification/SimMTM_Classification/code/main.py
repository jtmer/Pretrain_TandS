...
from dataloader import data_generator, simple_data_generator
...
parser.add_argument('--training_mode', default='pre_train', type=str, help='pre_train, fine_tune, train')
...
parser.add_argument('--pretrain_epoch', default=2, type=int, help='pretrain epochs')
...
def main(args, configs, seed=None):
    ...
    if args.training_mode == 'train':
        valid_dl, test_dl = simple_data_generator(targetdata_path, configs, training_mode,
                                                     subset=subset)
        train_dl = None
    else:
    ...
    # Trainer
    if args.training_mode == 'train':
        best_performance = SimpleTrainer(model, model_optimizer, model_scheduler, train_dl, valid_dl, test_dl, device, logger,
